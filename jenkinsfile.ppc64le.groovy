...
pipeline {
    ...
    stages {
        ...
        stage('Build') {
            steps {
                script {
                    kamelets_result = sh script: "./mvnw $MAVEN_PARAMS -Dmaven.test.failure.ignore=true clean install", returnStatus: true
                }
            }
            post {
                always {
    ...
    post {
        always {
            script {
                currentBuild.result = kamelets_result == 0 ? 'SUCCESS' : 'FAILURE'
            }
        }
    }
}