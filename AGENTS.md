...
- Camel: 4.20.0
- Java: 17+
- Build: Maven (`mvn verify`); Go is required for the `crds/` generator
...
- After adding or modifying a Kamelet, **regenerate and commit** the generated
  ...
  ```bash
  ./mvnw clean install   # regenerates nav.adoc and validates the catalog
  ```
...
# nav.adoc regeneration and catalog validation both run as part of the build
```
